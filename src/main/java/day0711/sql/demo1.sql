...
-- 2.1 all columns
/*
select id, username, password, name, gender, image, job, entry_date, create_time, update_time from emp;
 */
select id, username, password, name, gender, image, job, entry_date, create_time, update_time from emp;
...
select id,username,entry_date from emp order by entry_date asc;
/*
select * from emp order by entry_date asc ; -- 默认升序, asc可以省略的
 */
/*
 ...
 */
select id,username,entry_date from emp order by entry_date desc;

-- 3. 根据 入职时间 对公司的员工进行 升序排序 ， 入职时间相同 , 再按照 ID 进行降序排序
/*
select * from emp order by entry_date asc , id desc ;
 */
select id,username,entry_date from emp order by entry_date asc, id desc;
...
-- 练习 : 员工管理列表查询 , 根据最后操作时间, 进行倒序排序，分页，每页 10 条
-- 条件 : name , gender , entry_date

/*
select * from emp where name like '%张%' and gender = 1 and entry_date between '2000-01-01' and '2010-01-01' order by update_time desc limit 0,10 ;
 */
 ...
 */
select min(entry_date) 最早入职日期 from emp;

-- 4. 统计该企业最近入职的员工的入职日期
/*
select max(entry_date) from emp;
 */
select max(entry_date) 最近入职日期 from emp;
...
select job, count(*) as c from emp where entry_date <= '2015-01-01' group by job having c >= 2;
-- step 1
/*
select *  from emp where entry_date <= '2015-01-01';
 */
...
*/
select if(gender = 1, '男性员工', '女性员工') as 性别, count(*) as 人数 from emp group by gender;
